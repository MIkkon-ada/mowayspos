...
def upgrade() -> None:
    ...
    op.create_index("ix_execution_schedules_id", "execution_schedules", ["id"])
    op.create_index("ix_execution_schedules_subtask_id", "execution_schedules", ["subtask_id"])
    op.create_index("ix_execution_schedules_plan_type", "execution_schedules", ["plan_type"])
    op.create_index("ix_execution_schedules_assignee_id", "execution_schedules", ["assignee_id"])
    op.create_index("ix_execution_schedules_status", "execution_schedules", ["status"])
    op.create_index("ix_execution_schedules_is_deleted", "execution_schedules", ["is_deleted"])
    ...
    op.create_table(
        ...
        sa.Column("wecom_error", sa.Text(), nullable=False),
        sa.Column("created_at", sa.DateTime()), sa.Column("updated_at", sa.DateTime()),
        sa.UniqueConstraint("schedule_id", "reminder_kind", "due_on", "recipient_id", name="uq_execution_schedule_reminder"),
    )
    op.create_index("ix_execution_schedule_reminders_id", "execution_schedule_reminders", ["id"])
    op.create_index("ix_execution_schedule_reminders_schedule_id", "execution_schedule_reminders", ["schedule_id"])
    op.create_index("ix_execution_schedule_reminders_recipient_id", "execution_schedule_reminders", ["recipient_id"])
