...
| 2026-07-28 | 启动会确认 Agent | 增加 `pending_kickoff`：启动会审核通过前不得正式汇报或写入执行期任务；审核通过后原子切换为 `active`。 |

## 六、待启动会确认边界

- `pending_review` 经企业教练审核通过后进入 `pending_kickoff`，而不是直接进入 `active`。
- `pending_kickoff` 不允许正式汇报、执行期任务结构写入或创建普通会议纪要。
- PM 创建并提交启动会审核包；企业教练逐项审核提案。所有提案均通过后才能确认启动。
- 确认启动会在同一事务内写入已审核的变更、发布 `meeting_type=kickoff` 纪要、记录启动信息并将项目切换为 `active`。
