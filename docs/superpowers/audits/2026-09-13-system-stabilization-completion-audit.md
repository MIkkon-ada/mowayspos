...
| CI is fail-closed | `cloud-p1b2a-gate.yml` runs full backend tests, frontend tests/build, bundle baseline, PostgreSQL migration gate and Compose health checks. A source search found no allow-failure/whitelist/`continue-on-error` configuration. GitHub Actions run `34073302625` completed successfully on `9887e2c`, an ancestor of the current HEAD. | Historical remote run proven; current HEAD rerun pending |
| Alembic/ORM and fresh SQLite/PostgreSQL schema alignment | Fresh backend suite includes the database safety, Alembic and meeting-document-source governance tests. The CI workflow contains a PostgreSQL 16 empty-database `alembic upgrade head` gate; historical run `34073302625` passed that step. | SQLite/current local coverage and historical PostgreSQL gate proven; current HEAD pending |
| Core business smokes and rejected permission paths | Broad route/service permission regression tests pass locally. CI runs `scripts/ci_compose_business_smoke.py` after nginx health: historical run `34073302625` passed Compose smoke and port isolation, including the authenticated business paths. | Historical remote smoke proven; current HEAD rerun pending |
...
2. No remote CI run for the current HEAD has been observed. The latest successful hosted run is `34073302625` at `9887e2c`; the current branch is ahead of that commit and has no P1B2A run recorded.
