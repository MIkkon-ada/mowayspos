...
| CI is fail-closed | `cloud-p1b2a-gate.yml` runs full backend tests, frontend tests/build, bundle baseline, PostgreSQL migration gate and Compose health checks. A source search found no allow-failure/whitelist/`continue-on-error` configuration. Code/workflow commit `f69a1eb` passed hosted run `34739636224`; later `6792cf8` changes documentation only. | Proven |
| Alembic/ORM and fresh SQLite/PostgreSQL schema alignment | Fresh backend suite includes the database safety, Alembic and meeting-document-source governance tests. Hosted run `34739636224` on code/workflow commit `f69a1eb` passed the PostgreSQL 16 empty-database `alembic upgrade head` gate. | Proven |
| Core business smokes and rejected permission paths | Broad route/service permission regression tests pass locally. Hosted run `34739636224` on code/workflow commit `f69a1eb` passed `scripts/ci_compose_business_smoke.py`, nginx health, authenticated business paths and port isolation. | Proven |
