...
- [x] **Step 1: Add focused Vitest coverage.**
...
- [x] **Step 2: Add the source-boundary contract.**
...
- [x] **Step 1: Create `projectIdentity.ts`.**
...
- [x] **Step 2: Create `compatibility/projectNames.ts`.**
...
- [x] **Step 3: Turn `projectDisplay.ts` into a compatibility re-export.**
...
- [x] **Step 5: Commit the strict/compatibility split.**
...
- [x] **Step 1: Change `TaskManagementPage.tsx`.**
...
- [x] **Step 2: Change `exportTasksExcel.ts`.**
...
- [x] **Step 3: Expand the boundary test for both task consumers.**
...
- [x] **Step 5: Commit caller migration.**
...
- [x] **Step 1: Migrate direct display imports.**
...
- [x] **Step 2: Remove the transitional facade only when the search is empty.**
...
- [x] **Step 3: Run release-equivalent frontend verification.**
...
- The initial red run failed as intended: `getProjectGroupKey` and the explicit compatibility/identity modules did not exist. The task-consumer contract later failed as intended until both callers used the shared adapter.
- Focused compatibility behavior and source-boundary checks passed after the split: 4 Vitest tests and the Node contract test.
- A production build first exposed `TS2502` in the new test-only function type: its parameter name shadowed the outer `projects` fixture. Renaming only the parameter to `projectList` made `npx tsc -b --pretty false` pass; no production behavior changed.
- Final frontend verification: `npm run test:all` passed with 28 Vitest files / 93 tests and 508 contract tests. `npm run test:bundle` passed; initial JS was 302985 B (gzip 94487 B), CSS 97388 B (gzip 17223 B), largest route 95588 B (gzip 24537 B), and ExcelJS 940194 B (gzip 271326 B) remained non-initial.
- Source verification found no remaining `projectDisplay` import, and no `legacy:` or `task.special_project` occurrence in `TaskManagementPage.tsx` or `exportTasksExcel.ts`.
