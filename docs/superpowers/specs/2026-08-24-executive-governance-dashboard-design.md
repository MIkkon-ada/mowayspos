...
4. **重点 Initiative**：全宽紧凑列表以关键任务（`SubTask`）为行，并显示所属重点工作（`Task`）作为上下文；列为 Initiative、结果责任/协同、正式进展、下个节点、状态。协同人只显示最多三枚头像和总人数；Evidence 计数只在状态图标或简短计数中显示。
...
2. 每个 Initiative 行对应一个关键任务（`SubTask`），并可辨认所属重点工作、唯一责任人、协同规模、正式进展、下个节点和健康状态，且不出现解释段落。
