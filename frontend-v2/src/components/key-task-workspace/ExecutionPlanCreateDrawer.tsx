...
import { createMonthlyPlan, updateMonthlyPlan, type MonthlyPlanPayload } from '../../api/monthlyPlans'
...
function planForm(plan: ExecutionPlan, defaultAssigneeId: number | null): MonthlyPlanPayload {
  return {
    ...blankPlan(defaultAssigneeId),
    title: plan.title,
    expected_output: plan.expected_output,
    assignee_id: plan.assignee_id ?? defaultAssigneeId ?? 0,
    collaborator_ids: plan.collaborator_ids,
    status: plan.status as MonthlyPlanPayload['status'],
    start_date: plan.start_date,
    due_kind: plan.due_kind,
    due_date: plan.due_date,
    due_label: plan.due_label,
    due_reference_date: plan.due_reference_date,
    completion_criteria: plan.completion_criteria,
    progress_note: plan.progress_note,
    risk_dependency: plan.risk_dependency,
    actual_output: plan.actual_output,
  }
}

export function ExecutionPlanCreateDrawer({ keyTaskId, defaultAssigneeId, members, plan = null, onClose, onCreated }: Props) {
  const [form, setForm] = useState<MonthlyPlanPayload>(() => plan ? planForm(plan, defaultAssigneeId) : blankPlan(defaultAssigneeId))
  ...
  useEffect(() => {
    setForm(plan ? planForm(plan, defaultAssigneeId) : blankPlan(defaultAssigneeId))
    setMode('manual')
    setError('')
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (plan) await updateMonthlyPlan(plan.id, payload)
      else await createMonthlyPlan(keyTaskId, payload)
      onCreated()
      onClose()
    } catch (reason) {
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/25" role="dialog" aria-modal="true" aria-label={plan ? '编辑任务计划' : '新增任务计划'} onClick={onClose}>
    <form onSubmit={(event) => void submit(event)} className="flex h-full w-full max-w-[500px] flex-col bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
      <header className="flex items-start justify-between border-b border-slate-200 px-6 py-5">
        <div><h2 className="text-lg font-semibold text-slate-900">{plan ? '编辑任务计划' : '新增任务计划'}</h2><p className="mt-1 text-xs text-slate-400">为当前关键任务拆解一条可负责、可交付的计划。</p></div>
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-5">
        {!plan && <div className="flex gap-2 rounded-lg bg-slate-100 p-1">
        ...
        {mode === 'ai' ? proposalRun ? <TaskPlanProposalReview run={proposalRun} members={selectableMembers} busy={saving} onUpdate={updateDraft} onApply={applyDrafts} /> : <div className="space-y-4">
          ...
          <label className="block text-xs font-medium text-slate-600">上传附件<input aria-label="上传附件" type="file" accept={acceptedDocumentTypes('taskPlan')} multiple disabled={saving} className="mt-1 block w-full text-sm" onChange={(event) => {
            const incoming = Array.from(event.target.files ?? [])
            const invalid = incoming.find((file) => !aiDocumentFormats.taskPlan.some((extension) => file.name.toLowerCase().endsWith(extension)) || file.size > 10 * 1024 * 1024)
        ...
        {plan?.display_status === '已延期' && <Field label="延期原因"><textarea aria-label="延期原因" rows={2} value={form.delay_reason} disabled={saving} onChange={(event) => patch('delay_reason', event.target.value)} className={inputClass} /></Field>}
        </>}
      ...
      <footer className="flex justify-end gap-2 border-t border-slate-200 px-6 py-4">
        ...
        {mode === 'manual' && <button disabled={saving} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{saving ? (plan ? '保存中…' : '创建中…') : (plan ? '保存修改' : '创建计划')}</button>}
      </footer>
    </form>
  </div>
}
