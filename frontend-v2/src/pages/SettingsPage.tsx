...
export function SettingsPage() {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          ...
          >
            <path
              ...
              strokeLinejoin="round"
              strokeWidth="2.5"
              d={toast.ok ? "M5 13l4 4L19 7" : "M6 18L18 6M6 6l12 12"}
            />
          </svg>
          {toast.msg}
        </div>
      ...
      {/* Top Bar */}
      <header
        className="h-16 flex items-center px-6 gap-3 flex-shrink-0 bg-white border-b"
        style={{ borderColor: "#E9EFF6" }}
      >
      ...
      {loading ? (
        ...
        <div className="flex-1 overflow-hidden flex">
          {/* Secondary nav */}
          <div
            className="w-52 flex-shrink-0 bg-white border-r overflow-y-auto p-3"
            style={{ borderColor: "#E9EFF6" }}
          >
          ...
          >
            <div
              className={`${effectiveSection === "ai" ? "max-w-6xl" : "max-w-3xl"} mx-auto space-y-5`}
            >
              ...
              {effectiveSection === "notify" && (
                <Card>
                  ...
                  <Field
                    label="任务延期提醒"
                    desc="任务临近或超过计划时间时，向负责人推送提醒"
                  >
                    <Toggle checked={notifyDelay} onChange={setNotifyDelay} />
                  </Field>
                  <Field
                    label="AI 待确认提醒"
                    desc="AI 提取结果待确认时，每日汇总推送至确认人"
                  >
                    <Toggle checked={notifyAI} onChange={setNotifyAI} />
                  </Field>
                  <Field
                    label="需决策事项提醒"
                    desc="出现需高层决策的问题时，即时通知决策人"
                  >
                    <Toggle
                      checked={notifyDecision}
                      onChange={setNotifyDecision}
                    />
                  </Field>
                  <Field
                    label="周报自动汇总"
                    desc="每周五 18:00 自动生成项目周报并发送给管理层"
                  >
              ...
              {effectiveSection === "people-mgmt" && (
                <AccountPeopleMgmtSection />
              )}

              {(["integration", "data"] as Section[]).includes(
                effectiveSection,
              ) && (
                <Card>
                  <div className="py-12 flex flex-col items-center text-center">
                    ...
                    <p className="text-sm font-bold text-slate-700">
                      {
                        { integration: "集成与接口", data: "数据与备份" }[
                          effectiveSection as "integration" | "data"
                        ]
                      }
                    </p>
