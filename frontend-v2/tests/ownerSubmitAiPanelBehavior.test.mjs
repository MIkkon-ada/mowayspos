...
test('panel aborts analysis requests on close and ignores late results', () => {
  ...
  assert.match(source, /setPanelState\('failed'\)/)
})
...
test('panel cancels active upload, analysis, and polling requests during cleanup', () => {
  assert.match(source, /mountedRef\.current = false[\s\S]*uploadControllerRef\.current\?\.abort\(\)[\s\S]*analysisControllerRef\.current\?\.abort\(\)[\s\S]*clearPolling\(\)/)
  assert.match(source, /uploadControllerRef\.current\?\.abort\(\)/)
  assert.match(source, /analysisControllerRef\.current\?\.abort\(\)/)
  assert.doesNotMatch(source, /initializationControllerRef|getLatestInitAnalysisRun|listAttachmentsSafely/)
})
