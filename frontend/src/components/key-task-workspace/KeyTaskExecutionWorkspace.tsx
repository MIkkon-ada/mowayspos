...
export function KeyTaskExecutionWorkspace({ keyTaskId, onBack, projectId }: { keyTaskId: number; onBack?: () => void; projectId?: number | null }) {
  ...
  return <main className="flex-1 min-h-0 overflow-y-auto bg-slate-50"><div className="mx-auto max-w-[1440px] space-y-4 p-4 pb-24 min-[800px]:p-6">{onBack && <button type="button" onClick={onBack} className="text-sm font-medium text-slate-500 hover:text-blue-600">← 返回</button>}<KeyTaskHeader workspace={workspace} onSubmitUpdate={submitUpdate} onConfirmCompletion={() => void confirm()} onReopen={() => void reopen()} onChangeRisk={() => void changeRisk()} /><div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_330px]"><div className="space-y-4"><CurrentProgressCard progress={workspace.current_progress} /><ExecutionPlanTable plans={workspace.execution_plans} summary={workspace.plan_summary} onOpen={setSelectedPlan} /><AchievementList items={workspace.achievements} /><IssueList items={workspace.issues} /><ExecutionTimeline events={workspace.timeline} /></div><div className="xl:sticky xl:top-4 xl:self-start"><KeyTaskContextCard workspace={workspace} /></div></div></div><ExecutionPlanDetailDrawer open={Boolean(selectedPlan)} plan={selectedPlan} workspace={workspace} onClose={() => setSelectedPlan(null)} onSubmitUpdate={submitUpdate} onEdit={() => { setSelectedPlan(null); submitUpdate() }} onMarkCompleted={(plan) => void markPlanCompleted(plan)} /></main>
}
