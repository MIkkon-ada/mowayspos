...
export function KeyTaskExecutionWorkspace({ keyTaskId, onBack, projectId }: { keyTaskId: number; onBack?: () => void; projectId?: number | null }) {
  ...
  const changeRisk = async () => { const currentNote = workspace?.key_task.risk_note.trim() || ''; if (currentNote) { if (!window.confirm('确认解除该关键任务的风险标记？')) return; await setKeyTaskRisk(keyTaskId, ''); refresh(); return } const note = window.prompt('请填写风险原因'); if (!note?.trim()) return; await setKeyTaskRisk(keyTaskId, note.trim()); refresh() }
  ...
  return <main className="flex-1 min-h-0 overflow-y-auto bg-slate-50"><div className="mx-auto max-w-[1440px] space-y-5 pb-24">{onBack && <button type="button" onClick={onBack} className="ml-4 mt-4 text-sm font-medium text-slate-500 hover:text-blue-600 min-[800px]:ml-6">← 返回</button>}<KeyTaskHeader workspace={workspace} onSubmitUpdate={submitUpdate} onConfirmCompletion={() => void confirm()} onReopen={() => void reopen()} onChangeRisk={() => void changeRisk()} /><div className="grid gap-4 px-4 min-[800px]:px-6 xl:grid-cols-[minmax(0,1fr)_330px]"><div className="space-y-4"><CurrentProgressCard progress={workspace.current_progress} /><ExecutionPlanTable plans={workspace.execution_plans} summary={workspace.plan_summary} onOpen={setSelectedPlan} /><div className="grid gap-4 md:grid-cols-2"><AchievementList items={workspace.achievements} /><IssueList items={workspace.issues} /></div><ExecutionTimeline events={workspace.timeline} /></div><div className="xl:sticky xl:top-4 xl:self-start"><KeyTaskContextCard workspace={workspace} /></div></div></div><ExecutionPlanDetailDrawer open={Boolean(selectedPlan)} plan={selectedPlan} workspace={workspace} onClose={() => setSelectedPlan(null)} onSubmitUpdate={submitUpdate} onEdit={() => { setSelectedPlan(null); submitUpdate() }} onMarkCompleted={(plan) => void markPlanCompleted(plan)} /></main>
}
