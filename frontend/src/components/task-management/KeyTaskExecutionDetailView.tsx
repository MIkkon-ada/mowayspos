...
export function KeyTaskExecutionDetailView({ project, task, subTask, projectMembers, onBack, canManageSchedules = false, onSchedulesChanged }: Props) {
  ...
  const defaultAssigneeId = projectMembers.find((member) => member.person_name_snapshot === subTask.assignee)?.person_id ?? null
  ...
  return <main className="flex-1 overflow-y-auto bg-slate-50"><div className="mx-auto max-w-[1360px] px-6 py-5">
    ...
    <section className="mt-3 rounded-xl border border-slate-200 bg-white px-5 py-4"><div className="flex flex-wrap items-start justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h1 className="text-xl font-semibold tracking-tight text-slate-900">{subTask.title}</h1><span className="rounded-full bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-700">{subTask.status || '未开始'}</span></div><p className="mt-2 text-xs text-slate-500">所属重点工作：{task?.key_task || '—'}</p></div><span className="text-xs text-slate-400">所属项目：{project?.name || '—'}</span></div><div className="mt-4 grid border-t border-slate-100 pt-3 sm:grid-cols-2 xl:grid-cols-4"><Info title="直接负责人" value={subTask.assignee || '未指定'} />{collaborator && <Info title="协同人" value={collaborator} />}<Info title="整体状态" value={subTask.status || '未开始'} accent /><Info title="计划周期" value={subTask.plan_time || '未填写'} /></div></section>
