...
export function buildArchiveTimeline(input: {
  ...
  logs.forEach((log) => pushEvent(events, { id: `log-${log.id}`, title: operationTitle(log.action), detail: log.operator || '系统', at: log.created_at, tone: /archive|归档/i.test(log.action) ? 'green' : 'blue' }))
