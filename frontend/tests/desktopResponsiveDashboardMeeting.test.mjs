...
test('meeting uses wrapping desktop header and responsive content padding', () => {
  ...
  assert.match(meeting, /<main className="flex-1 overflow-y-auto p-4 lg:p-6"/)
})

test('selected meeting detail uses responsive five-track layout', () => {
  assert.match(meeting, /className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5 mb-5"/)
